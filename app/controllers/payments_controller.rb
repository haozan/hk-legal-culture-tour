class PaymentsController < ApplicationController
  ...
  def success
    ...
    # Success page will show subscription details
  end
  ...
  def set_payment
    # Verify payment belongs to current user if user is authenticated
    @payment = Payment.find(params[:id])
    
    if current_user && @payment.payable.respond_to?(:user)
      unless @payment.payable.user == current_user
