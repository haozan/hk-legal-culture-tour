class StripePaymentService < ApplicationService
  ...
  def self.process_payment_paid(payment)
    ...
    when 'Subscription'
      ...
      Rails.logger.info "Subscription #{subscription.id} activated for customer #{subscription.customer_email}"
      
      # TODO: Send confirmation email
      # SubscriptionMailer.activated(subscription).deliver_later
    
    # Add other payable types here as needed
    # when 'Order'
    #   order = payment.payable
    #   order.update!(status: 'paid')
    #   order.user.increment!(:credits, order.quantity)
    #   OrderMailer.confirmation(order).deliver_later
      ...
      Rails.logger.info "No specific business logic for payable type: #{payment.payable_type}"
    end
  end
end
